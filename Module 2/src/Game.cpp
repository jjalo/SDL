...
{
	// Load some graphics
	SDL_Texture* splashImage = graphicsManager->LoadTexture("res/splash.bmp");
	SDL_Texture* coverImage  = graphicsManager->LoadTexture("res/cover.png");
	SDL_Texture* pagesImage  = graphicsManager->LoadTexture("res/pages.png");
	...
	Vector2 startPosition = Vector2(GraphicsManager::SCREEN_WIDTH / 2.0, 
									GraphicsManager::SCREEN_HEIGHT / 2.0);

	player->SetPosition(startPosition);

	GetPlayer().SetSprite(player);
  ...
  while ( GetProperty("running") ) 
  {
	...
	cover->Draw(*graphicsManager);
	pages->Draw(*graphicsManager);
	GetPlayer().Draw(*graphicsManager);
	graphicsManager->Render();
...
Game::Execute(MoveCommand & cmd)
{
  ...
  else 
  {
    ...
    if ( pNext ) 
    {
		...
		if (cmd.m_Dir == 0) GetPlayer().MoveSteps(0, -1);
		else if (cmd.m_Dir == 1) GetPlayer().MoveSteps(0, 1);
		else if (cmd.m_Dir == 2) GetPlayer().MoveSteps(1, 0);
		else if (cmd.m_Dir == 3) GetPlayer().MoveSteps(-1, 0);
