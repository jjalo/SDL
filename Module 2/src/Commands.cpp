...
////////////////////////////////////////////////////////////////////////////////
Command *
CommandUtils::Parse(SDL_Event& event)
{
	Room *current = g_Game.GetCurrentRoom();
	...
	case SDL_KEYDOWN: case SDL_JOYHATMOTION: case SDL_JOYBUTTONDOWN:
		if (event.key.keysym.sym == SDLK_ESCAPE || event.jbutton.button == 8)
			return new QuitCommand();
		else if (event.key.keysym.sym == SDLK_UP || event.key.keysym.sym == SDLK_w || 
				event.jhat.value == SDL_HAT_UP)
		return new MoveCommand(North, g_Game.GetCurrentRoom(), current->GetNextRoom(North));
		else if (event.key.keysym.sym == SDLK_DOWN || event.key.keysym.sym == SDLK_s || 
			event.jhat.value == SDL_HAT_DOWN)
			return new MoveCommand(South, g_Game.GetCurrentRoom(), current->GetNextRoom(South));
		else if (event.key.keysym.sym == SDLK_LEFT || event.key.keysym.sym == SDLK_a || 
			event.jhat.value == SDL_HAT_LEFT)
			return new MoveCommand(West, g_Game.GetCurrentRoom(), current->GetNextRoom(West));
		else if (event.key.keysym.sym == SDLK_RIGHT || event.key.keysym.sym == SDLK_d || 
			event.jhat.value == SDL_HAT_RIGHT)
			return new MoveCommand(East, g_Game.GetCurrentRoom(), current->GetNextRoom(East));
		else 
			return new NullCommand();
		break;
